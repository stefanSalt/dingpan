...
- **右键菜单**：显示模式、添加品种、设置、检查更新、安装到系统、隐藏到托盘、置顶开关、退出。
- **系统托盘**（若环境支持）：双击图标显示/隐藏；右键同样有菜单。
- **设置**：刷新间隔 / 不透明度 / 字号 / 配色 / 置顶 / **开机自启（仅 Windows）** / **启动时最小化到托盘** / **启动时检查更新**。

---

## 更新与安装（Windows）

- **检查更新**：右键菜单 / 托盘「检查更新…」。程序读取本仓库最新 Release 比较版本；默认**启动时也会后台静默检查**（可在设置关闭）。
- **自动更新**：发现新版后点「更新并重启」，程序自动下载新 exe、就地替换并重启（原理：重命名运行中的 exe → 落入新文件 → 重启）。非打包 / 非 Windows 环境则改为打开下载页。
- **安装到系统**：右键菜单 / 托盘「安装到系统…」。把 exe 复制到 `%LOCALAPPDATA%\Dingpan`，并创建开始菜单 / 桌面快捷方式（**免管理员**）。**卸载**：删除该目录与对应快捷方式即可。
- **发布新版本**（维护者）：打 `vX.Y.Z` 标签推送，CI 自动构建并创建带 exe 的 Release，用户即可在程序内检查到。
...
最终用户下载 `Dingpan.exe`，**双击即可运行**，无需安装 Python。
...
| `start_hidden` | 启动时最小化到托盘（需托盘） | `true` / `false` |
| `check_update_on_start` | 启动时后台检查更新 | `true` / `false` |
